...
if 'story_planner_model_selection' not in st.session_state:
...
with tab2:
    ...
    if ollama_model_list: # Only show config if models are fetched successfully
        ...
        with st.expander("Writer Agent Configuration"): # Expander for Writer Agent config
            ...
            writer_model_selection = st.selectbox("Model", ollama_model_list, index=writer_model_list.index(st.session_state['writer_model_selection']) if st.session_state['writer_model_selection'] in ollama_model_list else 0, key="writer_model_selection") # Use dynamic model list, corrected variable name
...
with tab4:
    ...
    st.text_area("Final Chapter Outlines Output", value=st.session_state['outline_creator_output'], height=400) # Display Chapter Outlines
